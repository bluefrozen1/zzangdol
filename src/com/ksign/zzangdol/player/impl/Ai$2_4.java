...
import com.ksign.zzangdol.score.Score$2_4;

public class Ai$2_4 extends Player {

	Random random = new Random();
	public Ai$2_4(int mine, int opposite) {
		super(mine, opposite);
	}
	...
	public List<Integer> scoreEvaluation(int x, int y, Map<int[], Integer> addScoreMap) {
		...
		for(int i=0; i<attackScoreList.size(); i++) {
			if(threeThree && (attackScoreList.get(i) >= 40)) {
				...
				if(Score$2_4.THREE_THREE_ATTACK_SCORE > attackScoreList.get(i)) {
					attackScoreList.remove(i);
					attackScoreList.add(i, Score$2_4.THREE_THREE_ATTACK_SCORE);
				}
				break;
			}
		...
		for(int i=0; i<defenceScoreList.size(); i++) {
			if(threeThree && (defenceScoreList.get(i) >= 28)) {
				...
				if(Score$2_4.THREE_THREE_DEFENCE_SCORE > defenceScoreList.get(i)) {
					defenceScoreList.remove(i);
					defenceScoreList.add(i, Score$2_4.THREE_THREE_DEFENCE_SCORE);
				}
				break;
			}
	...
	private int evaluationPosition(int x, int y, String direction, boolean isAttack, Map<int[], Integer> addScoreMap) {
		...
		int[] scoreArray = null;
		Map<String, int[]> scoreMap = isAttack ? Score$2_4.getScoreMap() : Score$2_4.getDefenceScoreMap();
		for(int i=0; i<5; i++) {
	...
	private Map<int[], Integer> getAddScoreMap() {
		if(playStoneList.size() == 4 || playStoneList.size() == 5) {
			...
			if(firstMyStone[1] == secondMyStone[1]
				...
				addScoreMap.put(new int[]{leftStone[0] - 1, leftStone[1] - 1}, addScore);
				addScoreMap.put(new int[]{leftStone[0] - 1, leftStone[1] + 1}, addScore);
				addScoreMap.put(new int[]{leftStone[0], leftStone[1] - 2}, addScore);
				addScoreMap.put(new int[]{leftStone[0], leftStone[1] + 2}, addScore);
				addScoreMap.put(new int[]{rightStone[0] + 1, rightStone[1] + 1}, addScore);
				addScoreMap.put(new int[]{rightStone[0] + 1, rightStone[1] - 1}, addScore);
				addScoreMap.put(new int[]{rightStone[0], rightStone[1] - 2}, addScore);
				addScoreMap.put(new int[]{rightStone[0], rightStone[1] + 2}, addScore);
				return addScoreMap;
			...
			// 내 돌 2개가 새로로 나열된 경우
				...
				Map<int[], Integer> addScoreMap = new HashMap<>();
				addScoreMap.put(new int[]{topStone[0] - 1, topStone[1] - 1}, addScore);
				addScoreMap.put(new int[]{topStone[0] + 1, topStone[1] + 1}, addScore);
				addScoreMap.put(new int[]{topStone[0] - 2, topStone[1]}, addScore);
				addScoreMap.put(new int[]{topStone[0] + 2, topStone[1]}, addScore);
				addScoreMap.put(new int[]{bottomStone[0] - 2, bottomStone[1]}, addScore);
				addScoreMap.put(new int[]{bottomStone[0] + 2, bottomStone[1]}, addScore);
				addScoreMap.put(new int[]{bottomStone[0] - 1, bottomStone[1] + 1}, addScore);
				addScoreMap.put(new int[]{bottomStone[0] + 1, bottomStone[1] + 1}, addScore);
				return addScoreMap;
			...
			// 내 돌 2개가 대각선으로 나열된 경우
				...
				if(leftStone[1] < rightStone[1]) {
					addScoreMap.put(new int[]{leftStone[0] - 1, leftStone[1]}, addScore);
					addScoreMap.put(new int[]{leftStone[0] - 1, leftStone[1] + 1}, addScore);
					addScoreMap.put(new int[]{leftStone[0], leftStone[1] - 1}, addScore);
					addScoreMap.put(new int[]{leftStone[0], leftStone[1] + 2}, addScore);
					addScoreMap.put(new int[]{rightStone[0], rightStone[1] - 2}, addScore);
					addScoreMap.put(new int[]{rightStone[0] + 1, rightStone[1] - 1}, addScore);
					addScoreMap.put(new int[]{rightStone[0] + 1, rightStone[1]}, addScore);
					addScoreMap.put(new int[]{rightStone[0], rightStone[1] + 1}, addScore);
				} else {
					addScoreMap.put(new int[]{leftStone[0] - 1, leftStone[1] - 1}, addScore);
					addScoreMap.put(new int[]{leftStone[0] - 1, leftStone[1]}, addScore);
					addScoreMap.put(new int[]{leftStone[0], leftStone[1] - 2}, addScore);
					...
					addScoreMap.put(new int[]{rightStone[0] + 1, rightStone[1]}, addScore);
					addScoreMap.put(new int[]{rightStone[0] + 1, rightStone[1] + 1}, addScore);
					addScoreMap.put(new int[]{rightStone[0] + 0, rightStone[1] + 2}, addScore);
				}
				return addScoreMap;
			}
		}
