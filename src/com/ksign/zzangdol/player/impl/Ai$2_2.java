...
import com.ksign.zzangdol.score.Score$2_2;

public class Ai$2_2 extends Player {

	Random random = new Random();
	public Ai$2_2(int mine, int opposite) {
		super(mine, opposite);
	}
...
//			bestMove[1] = random.nextInt(10) + 5;
	...
	public List<Integer> scoreEvaluation(int x, int y) {
		...
		for(int i=0; i<attackScoreList.size(); i++) {
			if(threeThree && (attackScoreList.get(i) >= 45)) {
				System.out.println("3x3 Attack. x : " + x + ", y : " + y + ", myScoreList : " + attackScoreList);
				attackScoreList.remove(i);
				attackScoreList.add(i, Score$2_2.THREE_THREE_ATTACK_SCORE);
				break;
			}
			if(attackScoreList.get(i) >= 45) {
				threeThree = true;
			}
		}
		...
		for(int i=0; i<defenceScoreList.size(); i++) {
			if(threeThree && (defenceScoreList.get(i) >= 28)) {
				...
				defenceScoreList.add(i, Score$2_2.THREE_THREE_DEFENCE_SCORE);
				break;
			}
	...
	private int evaluationPosition(int x, int y, String direction, boolean isAttack) {
		...
		int[] scoreArray = null;
		Map<String, int[]> scoreMap = isAttack ? Score$2_2.getScoreMap() : Score$2_2.getDefenceScoreMap();
		for(int i=0; i<5; i++) {
